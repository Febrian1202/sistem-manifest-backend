<x-layout.app title="Laporan Kepatuhan" :breadcrumbs="[['name' => 'Dashboard', 'url' => route('dashboard')], ['name' => 'Pusat Laporan', 'url' => route('reports')], ['name' => 'Inventaris Software', 'url' => null]]">
    <div class="p-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
            <form action="{{ route('reports.software') }}" method="GET" class="flex flex-col md:flex-row items-stretch md:items-end gap-4">
            ...
            <div class="bg-amber-50 border-l-4 border-amber-400 p-4 mb-6 rounded-r-md">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-xs text-amber-700 mt-0.5">Daftar inventaris software pimpinan hanya memuat software yang terpasang pada komputer laboratorium yang telah disetujui (Approved) oleh Penanggung Jawab Laboratorium.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="mb-4 text-sm text-gray-600 italic">
            Menampilkan {{ $softwares->total() }} data untuk periode {{ $startDate->format('d/m/Y') }} s/d
                    ...
                    @foreach($softwares as $index => $sw)
                        ...
                        <tr class="{{ $isCritical ? 'bg-red-50' : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $softwares->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{ $sw->normalized_name }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Gratis
                                        / Tidak Perlu</span>
                                @endif
                            </td>
