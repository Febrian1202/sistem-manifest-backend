    ...
    <div class="p-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
            <form action="{{ route('reports.eksekutif') }}" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('reports.eksekutif') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-md">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Komputer</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $item['count'] }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $item['pct'] }}%</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Terinstall</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($topUnlicensed as $sw)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $sw->raw_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $sw->total }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="2" class="px-6 py-8 text-center text-gray-500 italic">Tidak ada data pelanggaran ditemukan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="mt-8 p-4 bg-blue-50 border border-blue-100 rounded-lg">
            <p class="text-sm text-blue-800">
                <strong>Catatan:</strong> Laporan ini merupakan ringkasan kondisi kepatuhan lisensi perangkat lunak di lingkungan institusi pada periode {{ \Carbon\Carbon::parse($startDate)->format('d/m/Y') }} s/d {{ \Carbon\Carbon::parse($endDate)->format('d/m/Y') }}.
            </p>
        </div>
    </div>
</x-layout.app>
