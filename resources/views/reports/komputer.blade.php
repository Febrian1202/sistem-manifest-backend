<x-layout.app>
    <div class="p-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
            <form action="{{ route('reports.komputer') }}" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('reports.komputer') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-md">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="mb-4 text-sm text-gray-600 italic">
            Menampilkan {{ $computers->total() }} data untuk periode {{ $startDate->format('d/m/Y') }} s/d {{ $endDate->format('d/m/Y') }}
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">IP / MAC</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="{{ $isInactive ? 'bg-red-50' : '' }}">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $computers->firstItem() + $index }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $computer->softwares_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 italic">{{ $computer->last_seen_at ? $computer->last_seen_at->diffForHumans() : '-' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
