<x-layout.app>
    <div class="p-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
            <form action="{{ route('reports.lisensi') }}" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('reports.lisensi') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-md">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="mb-4 text-sm text-gray-600 italic">
            Menampilkan {{ $licenses->total() }} data untuk periode {{ $startDate->format('d/m/Y') }} s/d {{ $endDate->format('d/m/Y') }}
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Quota</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Terpakai</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Sisa</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="{{ ($status === 'Penuh' || $status === 'Kedaluwarsa') ? 'bg-red-50' : '' }}">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $licenses->firstItem() + $index }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $license->catalog->normalized_name ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $license->quota_limit }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500 font-bold">{{ $license->used_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $license->remaining }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                            <div class="flex items-center justify-center">
                                <span class="text-xs font-semibold mr-2">{{ $license->usage_pct }}%</span>
                                <div class="w-16 bg-gray-200 rounded-full h-1.5">
                                    <div class="h-1.5 rounded-full {{ $license->usage_pct >= 90 ? 'bg-red-600' : ($license->usage_pct >= 75 ? 'bg-yellow-400' : 'bg-green-500') }}" style="width: {{ min(100, $license->usage_pct) }}%"></div>
                                </div>
                            </div>
                        </td>
