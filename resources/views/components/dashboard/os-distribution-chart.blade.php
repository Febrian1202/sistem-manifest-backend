...
<div class="bg-card border border-border rounded-lg p-5 shadow-sm" x-data="osChart(@js($series), @js($labels))">
    {{-- HTML Layout Tetap Sama --}}
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-foreground">Distribusi Sistem Operasi</h3>
    </div>
            ...
            <template x-for="(item, index) in chartData" :key="index">
                <div class="flex items-center justify-between group">
                    ...
                    <div class="text-right">
                        <span class="text-xs font-bold text-foreground" x-text="item.value + ' Unit'"></span>
...
<script>
    function osChart(seriesData, labelsData) {
        return {
            chart: null,
            series: seriesData || [],
            labels: labelsData || [],
            ...
            get chartData() {
                ...
                // PERBAIKAN 1: Masukkan semua warna yang ada di app.css (1-5)
                const colors = [
                    getVal('--chart-1'),
                    getVal('--chart-2'),
                    getVal('--chart-3'),
                ...
                return this.labels.map((label, index) => {
                    return {
                        ...
                        // PERBAIKAN 2: Gunakan Modulo (%) agar warna berulang (aman untuk data > 5)
                        color: colors[index % colors.length],
                        percent: total > 0 ? ((this.series[index] / total) * 100).toFixed(0) : 0
                    }
                });
            },
